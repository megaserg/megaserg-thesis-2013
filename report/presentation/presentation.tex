...
\begin{itemize}	
	\item Об эквивалентных контекстах: $\forall s\in\Sigma,\; \forall\omega,\:\omega^\prime\subseteq\Omega:$ если $s$ невырожденное, $gen(\omega,\{s\})$ определено и $B^0_{\omega} = B^0_{\omega^\prime}$, то и $gen(\omega^\prime,\{s\})$ определено, более того, $gen(\omega,\{s\}) = gen(\omega^\prime,\{s\})$
\end{itemize}
\end{frame}
...
\begin{itemize}
	\item Доказанная теорема конструктивна: предъявляется множество файлов, для~которых следует переиспользовать кэши, множество файлов, которые следует перекомпилировать, а~также контекст компиляции
	\item На~основе теоремы планируется реализовать поддержку переиспользования кэшей в~IntelliJ~IDEA
\end{itemize}
\end{frame}
